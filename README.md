...
Mercury-Unit デファクトスタンダードの録音再生ツール、Yas氏のSMR.XのKMD歌詞データファイルの簡易表示に対応しています。PCM/S44/A44だけでなくWAVやMP3でも利用できます。-xオプション利用時は全画面24x24ドットフォント、アートワークフルサイズで、-xの指定が無い場合は16x16ドットフォントでの簡易表示かつアートワークがハーフサイズになります。
